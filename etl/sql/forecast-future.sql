...
(
	...
	WHERE
		...
		AND T1.PROMOTION_ID NOT LIKE ALL ( 'DEFAULT', 'ARR-PLAN', 'Discontinuation PROFILE', 'APER%') 
		AND (T2.OVRD_PRESENTATION_QTY > 0 OR T2.OVRD_DISPLAY_QTY>0)
		AND PROMO_END_DATE >= (SELECT MIN(GD.GREGORIAN_DATE) FROM  GD) 
		AND T1.PROMO_START_DATE <= (SELECT MAX(GD.GREGORIAN_DATE) FROM GD)
...
-- Sumariza la consulta anterior obteniendo la maxima fecha de finalizaci{on de la Promo cuando ADDITIVE_SWITCH_CNT = 0
MAX_END_DDM_PF AS
(
	SELECT 
		M.LLAVE,
...
(
	...
	FROM   CALENDAR_DAY 
	WHERE
		CALENDAR_DAY.WM_MONTH =
		CASE
			WHEN (SELECT C.WM_MONTH + ?NUM_FUTMONTHS  FROM CALENDAR_DAY AS C WHERE C.GREGORIAN_DATE = DATE) > 12 
			THEN (SELECT C.WM_MONTH + ?NUM_FUTMONTHS  FROM CALENDAR_DAY AS C WHERE C.GREGORIAN_DATE = DATE) -12
			ELSE (SELECT C.WM_MONTH + ?NUM_FUTMONTHS  FROM CALENDAR_DAY AS C WHERE C.GREGORIAN_DATE = DATE) END

		AND CALENDAR_DAY.FISCAL_YEAR = 
			CASE
			WHEN (SELECT calendar_day.WM_MONTH + ?NUM_FUTMONTHS  FROM calendar_day WHERE calendar_day.GREGORIAN_DATE = DATE) > 12 
			THEN  (SELECT calendar_day.FISCAL_YEAR +1 FROM calendar_day WHERE calendar_day.GREGORIAN_DATE = DATE)
			ELSE  (SELECT calendar_day.FISCAL_YEAR FROM calendar_day WHERE calendar_day.GREGORIAN_DATE = DATE) END
	GROUP BY 1
),
--Consulta CTE_FCST
CTE_FCST AS 
...
(
	...
	(
		SELECT
			...
			0 AS PCT_WK,
			FCST_BASE * (DIAS/7) AS FCST_BASE_AJD,
			FCST_PROM * (DIAS/7) AS FCST_PROM_AJD 
		FROM FCST_B_P
	...
	(
		SELECT
			...
			FCST_BASE * PCT_WK * 1 AS FCST_BASE_AJD,
			FCST_PROM * PCT_WK * 1 AS FCST_PROM_AJD 
		FROM FCST_B_P
		...
		UNION 
			(
				SELECT
					...
					SAT, SUN, MON, TUE, WED, THU, FRI,
					((SAT_MULT * SAT + SUN_MULT * SUN + MON_MULT * MON + TUE_MULT * TUE + WED_MULT * WED + THU_MULT * THU + FRI_MULT * FRI) /100.0) AS PCT_WK,
					FCST_BASE * PCT_WK * 1.0 AS FCST_BASE_AJD,
					FCST_PROM * PCT_WK * 1.0 AS FCST_PROM_AJD 
				FROM FCST_B_P
				WHERE ALLOC_CAL_NAME IS NOT NULL
			)
	)  AS T
