#' Obtener los datos de forecast y ventas de los proximos 3 meses
#' requeridos para alimentar la aplicacion
#' 
#' Input:
#'   * Consultas de SQL:
#'      * inner_join_mes-actual.SQL
#'      * business_units.SQL
#'      * precio_futuro.SQL 
#'      * ventas.SQL
...
#'   
#' Output:
#'   * Archivo csv con el resumen obtenido a nivel vicepresidencia, departamento, categoria, 
#'   formato, fecha de calculo y ym (year-month)
#'   * Archivo csv con el detalle de los datos a nivel articulo formato
#' 
...
for(wm_month in 3:1)
{
  ...
  result <- left_join(query, precio_futuro, by = c('old_nbr'='item_nbr', 'formato')) %>%
    ...
    mutate(
      precio = coalesce(precio_fut, precio),
      precio_iva = coalesce(precio_iva_fut, precio_iva),
    ) %>%
    select(-c(precio_fut, precio_iva_fut)) %>%
    left_join(squads, by = c('dept_nbr', 'cat_nbr')) %>%
    mutate(
      forecast_base = fcst_base * precio,
      forecast_promocional = fcst_prom * precio,
      presentation = press_qty * precio,
      display = display_qty * precio * 1.00000,
      item1_desc = str_replace_all(item1_desc, '\\W', ''),
      fecha_calculo = today(),
      ym = as.numeric(format(today() %m+% months(wm_month), "%Y%m")),
      press_qty = replace_na(press_qty, 0),
      presentation = replace_na(presentation, 0)
    ) %>%
    rename( 
