...
log_path <- paste0(app_deployment_environment, '/log/')

pretty_text <- c(
  'user' = 'Usuarios únicos',
  'session' = 'Sesiones únicas'
)
  ...
  output$graph_daily <- renderPlot({
    if (is.null(logs_filt()) || nrow(logs_filt()) == 0) {
      ...
      if (input$split_by_clearance) {
        x <- logs_filt() %>% 
          mutate(
            color = names(clearance_levels)[match(map_dbl(role, ~role_clearance(.x, clearance_levels)), clearance_levels)]
          )
      } else {
        x <- logs_filt() %>% 
          mutate(
      ...
      p <- x %>%
        ...
        labs(
          title = pretty_text[input$variable],
          x = NULL,
          y = NULL
        )
    }
        ...
        tabPanel(
          ...
          fluidRow(
            column(
              ...
              selectInput(ns('variable'), lang$kpi, c('user', 'session') %>% set_names(pretty_text[.])),
              selectInput(ns('unit'), lang$unit, c('days', 'weeks', 'months', 'years') %>% set_names(get_pretty_names(.))),
